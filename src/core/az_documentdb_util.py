"""Azure Document DB utility for CRUD and vector search operations."""
    ...
    def ensure_vector_index(
        ...
        try:
            self.create_vector_index(
                collection_name, attr_name, index_name,
                index_type=index_type, similarity=similarity, dimensions=dimensions,
            )
        except OperationFailure as exc:
            if "already exists" in str(exc).lower() or exc.code == 68:
                logger.info("Vector index '%s' already exists on %s.%s", index_name, collection_name, attr_name)
            else:
                raise
